...
object ErrorLogger {
    ...
    fun getUserMessage(error: TranscriptionError): String {
        return when (error) {
            is TranscriptionError.NetworkError -> "网络连接失败，请检查网络设置后重试"
            is TranscriptionError.AuthError -> "API 密钥无效，请在设置中更新密钥"
            is TranscriptionError.AuthenticationError -> "API 密钥无效，请在设置中更新密钥"
            is TranscriptionError.ValidationError -> "输入验证失败：${error.message}"
            is TranscriptionError.FileError -> "文件错误：${error.message}"
            is TranscriptionError.ApiError -> when (error.code) {
                400 -> "请求格式错误，请重试"
                401 -> "API 密钥无效"
                403 -> "访问被拒绝，请检查权限"
                404 -> "服务未找到"
            ...
            is TranscriptionError.TimeoutError -> "请求超时，请检查网络连接后重试"
            is TranscriptionError.UnknownError -> "未知错误：${error.message}"
        }
    }
